...
def upsert_to_pinecone(
    ...
    # 2) Chunked report vectors
    report: str = event.get("report", "")
    if not report:
        logger.warning("No report for event %s – skipping chunk upsert", event["title"])
        return

    vectors: List[Tuple[str, List[float], Dict[str, Any]]] = []
    for idx, chunk in enumerate(chunk_text(report)):
    ...
    pinecone_index.upsert(namespace=REPORT_NAMESPACE, vectors=vectors)
    logger.info("Upserted %d report chunks for event %s", len(vectors), event["title"])
